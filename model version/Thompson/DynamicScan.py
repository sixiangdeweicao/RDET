...
def Scan_Feedback(xi, budget,now_budget, R, T,  end, args):
    ...
    for i in range(max(int(len(xi)*args.num_node_p), 1)):  # 更新节点的属性
        ...
        if node.immediate_sample_size > 0:
            ...
            if node.activate_node/node.immediate_sample_size>0.9:
...
def ReplaceDescendants(xi, args):
    ...
    for node in xi:
        ...
        if node.parent.DS.stack == node.DS.stack:
            ...
            parent.R = np.random.beta(max(parent.activate_node, 0)+1, max(0, parent.inactivate_node)+1) # 假定 父节点与子节点 空间大小与reward
...
def Start():
    ...
    parse.add_argument('--output',type=str, default="/home/szx/code/baseline/V4_Thompsom/result",help='output directory name')

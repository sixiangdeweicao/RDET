...
class TreeNode:
    ...
    def __init__(self,iplist,_partent=None):
        ...
        self.region_size = 0
        self.searched_dim = 0
        self.flag_child = 0  #用来存储子节点中未搜完的节点数目
        self.generated_address = None
        self.immediate_sample_size = 0 #用于存储此次采样数目
        self.activate_node = 0
        self.inactivate_node = 0
        self.all_childs = []
    ...
    def OutputNode(self):
        ...
        print('[+]DR:', self.DR)
        print('[+]Reward:', self.R)
        print('[+]searched_dim:', self.searched_dim)
        print('[+]searched_dim:', self.searched_dim)
        print('[+]region_size:', self.region_size)
        print('\n')

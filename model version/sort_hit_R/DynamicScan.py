...
def DynamicScan(root, args):
    ...
    while 1:
        ...
        if now_budget <= batch:
            ...
            epochs.append(epoch)
            break
    dict_hit_result = {"epoch":epochs,"hit_rate":hit_rates,"remain_time":Remaining_times,"scaned_number":scan_number, "activate_number":activate_number,"alpha_analyse":alpha_analyse}
...
def Scan_Feedback(xi, budget,now_budget, R, T,  end, args):
    ...
    detected_list = []  # 用于存放探测的地址 那些分配的数据太小的都在后面呢
    ...
    for i in range(max(int(len(xi)*args.num_node_p), 1)):  # 更新节点的属性
        ...
        if node.immediate_sample_size > 0:
            node.hit_rate = float(len(new_active_addrs)/node.immediate_sample_size)
            detected_list.append(node)

    detected_list = sorted(detected_list, key=lambda node: node.hit_rate, reverse=True)
    for i in range(len(detected_list)):
        detected_list[i].LaterPos = i

    LenOfDetectedList = len(detected_list)
    sum_alpha = 0.0
    for node in detected_list:
        alpha = abs(node.FormerPos-node.LaterPos)/LenOfDetectedList
        sum_alpha+=alpha
        node.R = alpha*node.hit_rate+(1-alpha)*node.R
    alpha_analyse.append(sum_alpha/len(detected_list))
    print('[+]alpha analyse:{} \n'
          .format(sum_alpha/len(detected_list)))
    xi = sorted(xi, key=lambda node: node.R, reverse=True)
...
def Start():
    ...
    parse.add_argument('--output',type=str, default="/home/szx/code/baseline/V4_sort_hit_R/result",help='output directory name')

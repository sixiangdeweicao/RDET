...
def DynamicScan(root, args):
    ...
    epoch = 0
    args.num_node_p = min(100/len(xi), 0.01)
    print("节点数目：", len(xi))
    print(args)
    li = []
    for node in xi:
        li.append(pow(args.delta, len(node.DR)))
    li_np = np.array(li)
    print("max:", li_np.max(), "min:", li_np.min(), "mean:", li_np.mean())
    while 1:
        epochs.append(epoch)
        xi, now_budget, R, T, reward_list, node_list = Scan_Feedback(xi, budget, now_budget, R, T, 0, args)
        ser_sto[epoch] = [node_list, reward_list]
        """
        存储一下序列文件
        """
        with open(output_dir+'/budget_{},num_node_p={},batch={},delta={},beta={}.json'.format(args.budget, args.num_node_p, args.batch_size, args.delta, args.beta), 'w') as f:
            json.dump(ser_sto, f)
    ...
    hit_result.to_csv(output_dir+'/budget_{},num_node_p={},batch={},delta={},beta={}.csv'.format(args.budget, args.num_node_p, args.batch_size, args.delta, args.beta),index=False)
...
def Scan_Feedback(xi, budget,now_budget, R, T,  end, args):
    ...
    active_addrs -= R.intersection(active_addrs)
    ...
    for i in range(max(int(len(xi)*args.num_node_p), 1)):  # 更新节点的属性
        ...
        if node.immediate_sample_size > 0:
            node.activate_node += len(new_active_addrs)
            node.inactivate_node += node.immediate_sample_size-len(new_active_addrs)
            node.R = np.random.beta(max(node.activate_node, 0)+1, max(node.inactivate_node, 0)+1)  # 使用 Thompsom 采样更新reward
    xi = sorted(xi, key=lambda node: node.R, reverse=True)
    return xi, now_budget, R, T, reward_list, node_list
    ...
    for node in xi:
        ...
        if node.parent.DS.stack == node.DS.stack:
            ...
            for node_i in sum_node:
                ...
                parent.activate_node += node_i.activate_node
                parent.inactivate_node += node_i.inactivate_node
                # del node_i
            parent.searched_dim = len(sum_child_DR)
            parent.DR = list(set(parent.DR) | sum_child_DR)
            parent.R = np.random.beta(max(parent.activate_node, 0)+1, max(parent.inactivate_node,0)+1) # 假定 父节点与子节点 空间大小与reward
...
def Start():
    ...
    parse.add_argument('--input', type=str, default="/home/szx/dataset/BGP_1k_1w_1.json",help='input IPv6 addresses')
    parse.add_argument('--output',type=str, default="/home/szx/code/baseline/V4_Thompsom_BGP/result",help='output directory name')
    parse.add_argument('--budget',type=int, default=1000, help='the upperbound of scan times')
    ...
    print(args.input)
    V = InputAddrs(args.input, args.delta, args)

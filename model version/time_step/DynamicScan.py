...
def DynamicScan(root, args):
    ...
    while 1:
        epochs.append(epoch)
        xi, now_budget, R, T = Scan_Feedback(xi, budget, now_budget, R, T, 0, epoch, args)
        if now_budget <= 0:
            break
        ...
        if now_budget <= batch:
            xi, now_budget, R, T = Scan_Feedback(xi, budget, now_budget, R, T, 1, epoch, args)
            epochs.append(epoch)
            break
...
def Scan_Feedback(xi, budget,now_budget, R, T,  end, epoch, args):
    """
    对队列xi中的所有结点进行一次扫描，
...
def Start():
    ...
    parse.add_argument('--output',type=str, default="/home/szx/code/baseline/V4_time_step/result",help='output directory name')
